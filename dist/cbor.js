 ...
 */
export function parseCborMap(bytes, pos = 0) {
    if (pos >= bytes.length)
        throw new Error("parseCborMap: unexpected end");
    const initial = bytes[pos];
    const major = initial >> 5;
    if (major !== 5)
        throw new Error("parseCborMap: not a CBOR map (major " + major + ")");
    const additional = initial & 0x1f;
    pos++;
    let count = -1;
    if (additional < 24)
