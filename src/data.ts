...
// ── Flat UPLC codec (minimal — decode/encode for parameter application) ──────

// AST node: tagged union using arrays for compactness
// [0, deBruijn] = Var, [1, term] = Delay, [2, term] = Lambda,
// [3, func, arg] = Apply, [4, type, value] = Const, [5, term] = Force,
// [6] = Error, [7, builtinTag] = Builtin, [8, index, terms[]] = Constr,
// [9, term, continuations[]] = Case
type FTerm = readonly unknown[];

class FlatDec {
  private d: Uint8Array; private x = 0; private a = 0x80;
  constructor(d: Uint8Array) { this.d = d; }
  popBit(): 0|1 { if (this.a < 1) { this.a = 0x80; this.x++; } const r = (this.d[this.x]! & this.a) ? 1 : 0; this.a >>= 1; return r; }
  popBits(n: number) { let v = 0; for (let i = 0; i < n; i++) v = (v << 1) | this.popBit(); return v; }
  popByte() { if (this.a !== 0x80) { this.a = 0x80; this.x++; } return this.d[this.x++]!; }
  skipByte() { if (this.a < 1) { this.a = 0x80; this.x++; } this.x++; this.a = 0x80; }

  decodeList<T>(fn: () => T): T[] { const r: T[] = []; while (this.popBit() === 1) r.push(fn()); return r; }
  decodeNat(): bigint {
    const chunks: number[] = [];
    let cont = this.popBit();
    while (cont === 1) { chunks.push(this.popBits(7)); cont = this.popBit(); }
    chunks.push(this.popBits(7));
    let v = 0n;
    for (let i = 0; i < chunks.length; i++) v += BigInt(chunks[i]!) << (BigInt(i) * 7n);
    return v;
  }
  decodeBS(): Uint8Array {
    this.skipByte();
    ...
    while (len > 0) {
      parts.push(this.d.slice(this.x, this.x + len)); this.x += len;
      len = this.d[this.x++]!;
    }
  ...
  decodeTerm(): FTerm {
    ...
    switch (tag) {
      ...
      case 3: { const f = this.decodeTerm(); return [3, f, this.decodeTerm()]; }
      case 4: {
      ...
      case 9: { const t = this.decodeTerm(); return [9, t, this.decodeList(() => this.decodeTerm())]; }
      default: throw new Error("flat: tag " + tag);
    }
  }

  private decodeCV(w: number[], i: number): [unknown, number] {
    const t = w[i]!;
    if (t === 7) {
      const n = w[i+1]!;
      if (n === 5) { // list
    ...
    switch (t) {
      case 0: { const nat = this.decodeNat(); return [nat % 2n === 0n ? nat / 2n : -((nat + 1n) / 2n), i+1]; }
  ...
  private skipTE(w: number[], i: number): number {
    if (w[i] === 7) { const n = w[i+1]!; if (n === 5) return this.skipTE(w, i+2); if (n === 7 && w[i+2] === 6) return this.skipTE(w, this.skipTE(w, i+3)); }
    return i + 1;
  }
}

class FlatEnc {
  private b = new Uint8Array(256); private n = 0; private c = 0; private i = 0;
  private g() { const t = this.b; this.b = new Uint8Array(t.length * 2); this.b.set(t); }
  pushBit(v: 0|1) { this.c = (this.c << 1) | v; if (++this.i >= 8) { if (this.n >= this.b.length) this.g(); this.b[this.n++] = this.c; this.c = 0; this.i = 0; } }
  pushBits(v: number, n: number) { for (let j = n - 1; j >= 0; j--) this.pushBit(((v >> j) & 1) as 0|1); }
  pushByte(v: number) { if (this.n >= this.b.length) this.g(); this.b[this.n++] = v; }
  pushBytes(d: Uint8Array) { while (this.n + d.length > this.b.length) this.g(); this.b.set(d, this.n); this.n += d.length; }
  pad() { if (this.i === 0) { this.pushByte(1); return; } while (this.i < 7) this.pushBit(0); this.pushBit(1); }
  bytes(): Uint8Array { return this.b.slice(0, this.n); }

  encList<T>(items: T[], fn: (t: T) => void) { for (const it of items) { this.pushBit(1); fn(it); } this.pushBit(0); }
  encNat(n: bigint) {
    if (n < 0n) throw new Error("flat: negative nat");
    const chunks: number[] = [];
    if (n <= 127n) { chunks.push(Number(n)); }
    else { let v = n; while (v > 0n) { chunks.push(Number(v & 0x7fn)); v >>= 7n; } }
  ...
  encBS(d: Uint8Array) {
    this.pad();
    if (d.length === 0) { this.pushByte(0); return; }
  ...
  encTerm(t: FTerm): void {
    const tag = t[0] as number;
    this.pushBits(tag, 4);
    switch (tag) {
      case 0: this.encNat((t[1] as bigint) + 1n); break;
      case 1: this.encTerm(t[1] as FTerm); break;
      case 2: this.encTerm(t[1] as FTerm); break;
      case 3: this.encTerm(t[1] as FTerm); this.encTerm(t[2] as FTerm); break;
      case 4: {
        const wt = t[1] as number[];
        this.encList(wt, v => this.pushBits(v, 4));
        this.encCV(wt, 0, t[2]);
        break;
      }
      case 5: this.encTerm(t[1] as FTerm); break;
      case 6: break;
      case 7: this.pushBits(t[1] as number, 7); break;
      case 8: this.encNat(t[1] as bigint); this.encList(t[2] as FTerm[], tt => this.encTerm(tt)); break;
      case 9: this.encTerm(t[1] as FTerm); this.encList(t[2] as FTerm[], tt => this.encTerm(tt)); break;
    }
  }

  private encCV(w: number[], i: number, v: unknown): number {
    const t = w[i]!;
    if (t === 7) {
      const n = w[i+1]!;
      if (n === 5) { // list
        const s = i + 2;
        this.encList(v as unknown[], item => this.encCV(w, s, item));
        return this.skipTE(w, s);
      }
      if (n === 7 && w[i+2] === 6) { // pair
        const a = i+3, b = this.skipTE(w, a);
        const [fst, snd] = v as [unknown, unknown];
        this.encCV(w, a, fst);
        this.encCV(w, b, snd);
    ...
    switch (t) {
      case 0: { const n = v as bigint; this.encNat(n >= 0n ? n * 2n : -n * 2n - 1n); break; }
      case 1: this.encBS(v as Uint8Array); break;
      case 2: this.encBS(v as Uint8Array); break;
      case 3: break;
      case 4: this.pushBit((v as boolean) ? 1 : 0); break;
      case 8: this.encBS(v as Uint8Array); break;
 ...
 */
export function applyParamsToScript(compiledCode: string, params: PlutusField[]): string {
  if (params.length === 0) return compiledCode;

  const outerBytes = hexToBytes(compiledCode);
  let flat = decodeCbor(outerBytes, 0).value as Uint8Array;
  ...
  for (const param of params) {
    ...
    const constNode: FTerm = [4, [8], paramCbor];
    body = [3, body, constNode]; // Apply(body, const)
  }

  // Re-encode
  const enc = new FlatEnc();
  enc.pushByte(v0); enc.pushByte(v1); enc.pushByte(v2);
  enc.encTerm(body);
  enc.pad();
